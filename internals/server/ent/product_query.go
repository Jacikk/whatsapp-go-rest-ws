...
func (pq *ProductQuery) QueryAccesskeys() *AccessKeysQuery {
	query := (&AccessKeysClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withAccesskeys: pq.withAccesskeys.Clone(),
...
func (pq *ProductQuery) WithAccesskeys(opts ...func(*AccessKeysQuery)) *ProductQuery {
	query := (&AccessKeysClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withAccesskeys = query
	return pq
}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := pq.withAccesskeys; query != nil {
		if err := pq.loadAccesskeys(ctx, query, nodes, nil,
			func(n *Product, e *AccessKeys) { n.Edges.Accesskeys = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProductQuery) loadAccesskeys(ctx context.Context, query *AccessKeysQuery, nodes []*Product, init func(*Product), assign func(*Product, *AccessKeys)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Product)
	for i := range nodes {
...
func (pq *ProductQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withAccesskeys != nil {
			_spec.Node.AddColumnOnce(product.FieldAccessKeyID)
		}
	}
