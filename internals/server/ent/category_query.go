...
type CategoryQuery struct {
	...
	withAccesskeys *AccessKeysQuery
...
// QueryAccesskeys chains the current query on the "accesskeys" edge.
func (cq *CategoryQuery) QueryAccesskeys() *AccessKeysQuery {
	query := (&AccessKeysClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CategoryQuery) Clone() *CategoryQuery {
	...
	return &CategoryQuery{
		...
		withAccesskeys: cq.withAccesskeys.Clone(),
...
// the "accesskeys" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CategoryQuery) WithAccesskeys(opts ...func(*AccessKeysQuery)) *CategoryQuery {
	query := (&AccessKeysClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withAccesskeys = query
	return cq
}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withAccesskeys; query != nil {
		if err := cq.loadAccesskeys(ctx, query, nodes, nil,
			func(n *Category, e *AccessKeys) { n.Edges.Accesskeys = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CategoryQuery) loadAccesskeys(ctx context.Context, query *AccessKeysQuery, nodes []*Category, init func(*Category), assign func(*Category, *AccessKeys)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Category)
	for i := range nodes {
		if nodes[i].AccessKeyID == nil {
			continue
		}
		fk := *nodes[i].AccessKeyID
	...
	query.Where(accesskeys.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "access_key_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CategoryQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withAccesskeys != nil {
			_spec.Node.AddColumnOnce(category.FieldAccessKeyID)
		}
	}
